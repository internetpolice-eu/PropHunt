...
public class MainShop implements Listener
{
    ...
    public void openMainShop(final Player p) {
        final Inventory inv = Bukkit.createInventory((InventoryHolder)p, 9, MessageBank.SHOP_TITLE.getMsg());
        ...
        disguiseMeta.setLore((List)disLore);
        ...
        placeMeta.setLore((List)placeLore);
        ...
        loadMeta.setLore((List)loadLore);
    ...
    public void onInventoryClick(final InventoryClickEvent e) {
        if (this.inMenu.contains(e.getWhoClicked()) && e.getCurrentItem() != null && !e.getCurrentItem().getType().equals((Object)Material.AIR)) {
            if (e.getCurrentItem().getType().equals((Object)Material.ENDER_CHEST)) {
                e.setCancelled(true);
                e.getView().close();
                this.plugin.getServer().getScheduler().runTaskLater((Plugin)this.plugin, (Runnable)new BukkitRunnable() {
                    public void run() {
                        MainShop.this.plugin.getShopManager().getBlockChooser().openBlockShop((Player)e.getWhoClicked());
                    }
                }, 2L);
            }
            else if (e.getCurrentItem().getType().equals((Object)Material.GOLD_BLOCK)) {
                e.setCancelled(true);
                e.getView().close();
                this.plugin.getServer().getScheduler().runTaskLater((Plugin)this.plugin, (Runnable)new BukkitRunnable() {
                    public void run() {
            ...
            else if (e.getCurrentItem().getType().equals((Object)Material.DIAMOND_SWORD)) {
                e.setCancelled(true);
                e.getView().close();
                this.plugin.getServer().getScheduler().runTaskLater((Plugin)this.plugin, (Runnable)new BukkitRunnable() {
                    public void run() {
            ...
            else if (e.getCurrentItem().getType().equals((Object)Material.CHEST)) {
                e.setCancelled(true);
                e.getView().close();
                this.plugin.getServer().getScheduler().runTaskLater((Plugin)this.plugin, (Runnable)new BukkitRunnable() {
                    public void run() {
