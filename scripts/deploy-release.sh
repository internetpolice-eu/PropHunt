...
if [[ "$TRAVIS_PULL_REQUEST" == "false" ]]; then
  ...
  $HOME/build/internetpolice-eu/PropHunt/target/mvn-repo/ \
  ...
  $HOME/build/internetpolice-eu/PropHunt/target/site/apidocs/ \
  travis@travis.internetpolice.eu:WWW/javadocs/PropHunt/master/

  echo -e "Publishing final plugin release...\n"

  rsync -r --quiet -e "ssh -p 2222 -o StrictHostKeyChecking=no" \
  $HOME/build/internetpolice-eu/PropHunt/target/PropHunt-*.jar \
  travis@travis.internetpolice.eu:WWW/downloads/PropHunt/
